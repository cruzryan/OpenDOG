# dash_web.py
# Imports and global configs (MOTOR_CONFIG, LEGS_ORDER, DEFAULTS) - SAME AS PREVIOUS
# ... (ensure all these are correctly defined as in the last full dash_web.py I gave)
import os
import sys
import time
import threading
...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'quad_dashboard_secret_final_fix_attempt!' 
...
DEFAULT_MOTOR_PINS = [
    (39, 40, 41, 42),  # Original Motor 0 (e.g., FL_knee)
    (16, 15, 7, 6),    # Originally Motor 3's pins (e.g., FL_tigh), now at index 1
    (17, 18, 5, 4),    # Original Motor 2 (e.g., FR_knee)
    (37, 38, 1, 2),    # Originally Motor 1's pins (e.g., FR_tigh), now at index 3
    (37, 38, 1, 2),    # Original Motor 4 (e.g., BR_knee)
    (40, 39, 42, 41),  # Original Motor 5 (e.g., BR_tigh)
    (15, 16, 6, 7),    # Original Motor 6 (e.g., BL_knee)
    (18, 17, 4, 5),    # Original Motor 7 (e.g., BL_tigh)
]

DEFAULT_P = 0.9; DEFAULT_I = 0.0; DEFAULT_D = 0.3; DEFAULT_DEAD_ZONE = 5; DEFAULT_POS_THRESH = 5
actual_robot_control_is_on = False 

def perform_robot_initialization(robot_body: QuadPilotBody): # Unchanged from previous
        ...
        if body_monitor and not body_monitor._is_closed:
            ...
            for esp_idx in range(len(body_monitor.ips)):
                ...
                esp_packet = {
                    ...
                    "data_available": body_monitor.is_data_available_from_esp(esp_idx),
                    "mpu_available": False, "imu": {}, "motors": [],
                    "esp_control_actually_on": body_monitor.is_esp_control_reported_on(esp_idx), # From ESP broadcast
                    "data_age_ms": data_age # Latency/freshness for this ESP's data
                }
                if esp_packet["data_available"]:
                    esp_packet["mpu_available"] = body_monitor.is_mpu_available_for_esp(esp_idx)
                    if esp_packet["mpu_available"]:
                        esp_packet["imu"] = body_monitor.get_latest_imu_data_for_esp(esp_idx)
                    ...
                    if motor_data_raw:
                        for motor_local_idx in range(NUM_MOTORS_PER_ESP):
                            ...
                            angle = motor_data_raw['angles'][motor_local_idx] if motor_local_idx < len(motor_data_raw.get('angles',[])) else 0
                            esp_packet["motors"].append({ "id": global_idx, "name": name, "angle": angle })
...
# @app.route('/'), @socketio.on('connect'), @socketio.on('request_robot_init'), 
# @socketio.on('request_toggle_control'), @socketio.on('request_reset_all_hw'),
# @socketio.on('request_set_angles'), @socketio.on('disconnect')
# These should remain IDENTICAL to your last working version of dash_web.py that correctly handled these.
# The key is that `data_broadcasting_thread` now correctly includes `data_age_ms` for each ESP.
